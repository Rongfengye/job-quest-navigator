-- Update the check_user_monthly_usage function to use 3,3 limits for basic users
...
BEGIN
  ...
  IF usage_type = 'behavioral' THEN
    ...
    limit_count := 3; -- Basic users get 3 behavioral practices
  ELSIF usage_type = 'question_vault' THEN
    current_count := usage_record.question_vaults_count;
    limit_count := 3; -- Basic users get 3 question vault generations
  ELSE
...
-- Update the get_user_monthly_usage_summary function to use 3,3 limits for basic users  
...
BEGIN
  ...
  IF usage_record IS NULL THEN
    RETURN json_build_object(
      ...
      'behavioral', json_build_object(
        'current', 0,
        'limit', 3, -- Basic users get 3 behavioral practices
        'remaining', 3
      ),
      'questionVault', json_build_object(
        'current', 0,
        'limit', 3, -- Basic users get 3 question vault generations
        'remaining', 3
      ),
